...
function fixAsyncParams(filePath) {
  try {
    ...
    content = content.replace(
      /export async function (GET|POST|PUT|DELETE|PATCH)\s*\(\s*request:\s*NextRequest,\s*{\s*params\s*}\s*:\s*{\s*params:\s*{\s*([^}]+)\s*}\s*}\s*\)/g,
  ...
  { params }: { params: Promise<{ ${paramsType} }> }
    ...
    content = content.replace(
      /(\s*)\{\s*params\s*\}\s*:\s*{\s*params:\s*Promise<\{([^}]+)\}>\s*}\s*\)\s*\{[\s\n]*try\s*\{/g,
      (match, indent, paramsType) => {
